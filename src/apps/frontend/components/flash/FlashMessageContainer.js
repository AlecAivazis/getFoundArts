...
import removeFirstFlashMessage from 'actions/flashMessages/popMessage'
...
class FlashMessageContainer extends React.Component {
    ...
    constructor(props, ...args) {
        ...
        this.state = {
            // the timer should start off active if there are already
            // flash messages to show
            isTimerActive: props.flashMessages.length > 0,
        }
    }
    ...
    popMessage() {
        ...
        this.props.dispatch(removeFirstFlashMessage())
    }
     ...
     */
    componentWillReceiveProps({flashMessages}) {
        // make sure the timer is active
        this.setState({
            isTimerActive: flashMessages.length > 0,
        })
    }
    ...
    render () {
        ...
        return (
            ...
            >
                ...
                {
                    flashMessages.slice(0, 5).map((message, index) =>
                        <FlashMessage
                            key={index}
                            {...message}
                        />
